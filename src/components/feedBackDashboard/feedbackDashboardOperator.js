...
import ArrowPieChart from "./arrowPieChart";
...
const FeedbackDashboardSupervisor = () => {
  ...
  useEffect(() => {
    ...
    const fetchSummary = async () => {
      try {
        const res = await axios.get(`/getFeedbackSummary?date=${today}&line=1`);
    ...
    const fetchCategorySummary = async () => {
      try {
        const categorySummaryDatares = await axios.get(
          `/getFeedbackCategorySummary?date=${today}&line=1`
        );
    ...
    const fetchSeverityCounts = async () => {
      try {
        const severityDataRes = await axios.get(
          `/getFeedbackSeveritySummary?date=${today}&line=1`
        );
        const severityDataResult = severityDataRes.data;

        if (severityDataResult.status === "success") {
          if (severityDataResult.data.length > 0) {
            const severityData = severityDataResult.data[0];

            setSeverityData({
              major: parseInt(severityData.major),
              minor: parseInt(severityData.minor),
              critical: parseInt(severityData.critical),
            });
          } else {
            setSeverityData({
              major: 0,
              ...
              <div className="row pb-4 mt-0 px-3 pt-1">
                ...
                <div className="col-4 ">
                  <div className="custom-piechart-bg outline-border-chart">
                    <div className="align-items-center my-2 px-4 mb-4"></div>

                    <ArrowPieChart data={severityData} />
                  </div>
                </div>
              </div>
            </div>
